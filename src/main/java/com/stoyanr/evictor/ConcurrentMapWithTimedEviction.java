...
/**
 * A {@link java.util.concurrent.ConcurrentMap} that supports timed entry eviction. It provides
 * overloaded <tt>put</tt>, <tt>putIfAbsent</tt>, and <tt>replace</tt> methods with one additional
 * parameter, <tt>evictMs</tt>, which is the time in ms during which the entry can stay in the map
 * (time-to-live).
 * 
...
public interface ConcurrentMapWithTimedEviction<K, V> extends ConcurrentMap<K, V> {
     ...
     * (optional operation). If the map previously contained a mapping for the key, the old value is
     * replaced by the specified value. (A map <tt>m</tt> is said to contain a mapping for a key
     * <tt>k</tt> if and only if {@link #containsKey(Object) m.containsKey(k)} would return
     * <tt>true</tt>.)
     * 
